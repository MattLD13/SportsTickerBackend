...
class SportsFetcher:
    ...
    def fetch_all_teams(self):
        try:
            ...
            # 2. HARDCODE COLLEGE (SAFE FALLBACK)
            ...
            # 3. ENRICH COLLEGE (Background Fetch)
            try:
    ...
    def get_real_games(self):
        ...
        for league_key, config in self.leagues.items():
            ...
            if config.get('type') == 'leaderboard':
                # Racing Leaderboards... (omitted for brevity, handled same as before)
                continue
            ...
            try:
                ...
                for e in data.get('events', []):
                    ...
                    if 'soccer' in league_key:
                        h_score = re.sub(r'\s*\(.*?\)', '', h_score); a_score = re.sub(r'\s*\(.*?\)', '', a_score)
                    ...
                    game_obj = {
                        ...
                        'startTimeUTC': e['date'],
                        'situation': { 'possession': poss, 'isRedZone': sit.get('isRedZone', False), 'shootout': shootout_data }
                    }
...
def api_config():
    try:
        ...
        with data_lock:
            ...
            if was_demo and not is_demo: state['current_games'] = [] # INSTANT CLEAR
        save_config_file()
...
def list_tickers():
    ...
    for uid, rec in tickers.items():
        if cid in rec.get('clients', []):
            res.append({ "id": uid, "name": rec.get('name', 'Ticker'), "settings": rec['settings'], "last_seen": rec.get('last_seen', 0) })
    return jsonify(res)
