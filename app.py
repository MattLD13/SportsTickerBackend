...
# ================= TEAMS & LOGOS =================
# College teams require manual lists to filter from the massive NCAA ID pool
...
def generate_demo_data():
    return [
        ...
        # 3. Olympic Hockey
        {
            'type': 'scoreboard', 'sport': 'hockey_olympics', 'id': 'demo_oly', 'status': 'P3 4:20', 'state': 'in', 'is_shown': True,
            ...
            'away_abbr': 'USA', 'away_score': '5', 'away_logo': 'https://a.espncdn.com/i/teamlogos/countries/500/usa.png', 'away_color': '#002868', 'away_alt_color': '#BF0A30',
            'startTimeUTC': dt.now(timezone.utc).isoformat(), 'estimated_duration': 150,
...
class SportsFetcher:
    def __init__(self, initial_loc):
        ...
        self.leagues = {
            ...
            # --- OLYMPIC HOCKEY ---
            'hockey_olympics': { 'path': 'hockey/olympics.mens.ice_hockey', 'team_params': {'limit': 50}, 'type': 'scoreboard' },
        ...
        try:
            ...
            for league_key, config in self.leagues.items():
                if config['type'] == 'scoreboard' and 'team_params' in config:
                    # Skip college here, handle separately
            ...
            # 2. Fetch College Football (FBS/FCS)
    ...
    def fetch_shootout_details(self, game_id, sport='nhl'):
        ...
        try:
            ...
            if 'hockey' in sport:
                ...
                # ESPN sometimes puts shootout data in a specific structure for hockey
            ...
            # --- SOCCER PARSING ---
                ...
                # Soccer typically puts penalties in the 'shootout' list inside the boxscore or pbp
                # This is a heuristic based on common ESPN summaries
                shootout_plays = data.get("shootout", [])
                if not shootout_plays: return None
                
                for p in shootout_plays:
                    # 1 = goal, 2 = miss? (Values vary, simpler to check 'text')
