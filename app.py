...
# Sports: Fast updates (5s) for live scores
SPORTS_UPDATE_INTERVAL = 5       
# Stocks: Slower updates (60s) because Finnhub free tier is limited
STOCKS_UPDATE_INTERVAL = 60       
...
if os.path.exists(CONFIG_FILE):
    try:
        with open(CONFIG_FILE, 'r') as f:
            loaded = json.load(f)
            # Cleanup deprecated keys if any
...
def clean_my_teams(raw_list):
    if not isinstance(raw_list, list): return None
    cleaned = []
    seen = set()
...
class StockFetcher:
    ...
    def _rotate_key(self):
        ...
        # print(f"🔄 Rotating to Finnhub Key #{self.current_key_index + 1}")
    ...
    def _fetch_single_stock(self, symbol):
        ...
        try:
            ...
            if r.status_code == 429:
                # print(f"⚠️ Rate limit hit for key ...{api_key[-4:]}. Rotating...")
                self._rotate_key()
                return None
    ...
    def update_market_data(self, active_lists):
        ...
        # 2. Sequential Fetch (REQUIRED for Finnhub Free Tier)
        # We cannot use ThreadPoolExecutor here because 60 calls/min is strictly enforced.
        updated_count = 0
        ...
        for symbol in symbols_list:
            ...
            # ⚠️ CRITICAL: Sleep to respect rate limits.
            # 1.05 seconds ensures we stay under 60 calls/minute per key
            # With rotation, we could technically go faster, but let's be safe.
            time.sleep(1.05) 
...
class SportsFetcher:
    ...
    def lookup_team_info_from_cache(self, league, abbr, name=None):
        ...
        # Check Hardcoded Fallback for Soccer first
        if 'soccer' in league:
    ...
    def parse_shootout(self, raw, home_id=None, away_id=None, general_home=None, general_away=None):
        ...
        for hk, ak in (("home", "away"), ("homeTeam", "awayTeam"), ("homePenalties", "awayPenalties")):
            if hk in raw or ak in raw:
                return (self.parse_side_list(raw.get(hk) or []), self.parse_side_list(raw.get(ak) or []), pen_home_score, pen_away_score)
    ...
    def _format_live_clock(self, status: dict, fallback_text: str = "") -> str | None:
        ...
        if isinstance(live_time, dict):
            ...
            if long_val:
                ...
                if clock_match:
                    return _render_clock(int(clock_match.group(1)), int(clock_match.group(2)), max_time)
    ...
    def _extract_matches(self, sections, internal_id, conf, start_window, end_window, visible_start_utc, visible_end_utc):
        ...
        for section in sections:
            ...
            for match in candidate_matches:
                ...
                score_str_sources = [
                    status.get("scoreStr"), (match.get("home") or {}).get("scoreStr"), (match.get("away") or {}).get("scoreStr"),
                ...
                elif status.get("cancelled") or "Postponed" in reason or "PPD" in reason:
                    gst = 'post'
                    disp = "Postponed"
                ...
                matches.append({
                    'type': 'scoreboard',
                    'sport': internal_id, 'id': str(mid), 'status': disp, 'state': gst, 'is_shown': is_shown,
