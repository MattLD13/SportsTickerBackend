...
class EspnNewsSource:
    """Read recent ESPN team headlines for the V1 half-panel overlay."""
    ...
    def fetch(self, settings: DisplaySettings) -> Mapping[str, object]:
        """Return only headlines tied to teams selected on this ticker."""
        ...
        for league, url in self._news_urls.items():
            if not any(value.startswith(f"{league}:") for value in followed):
                continue
            try:
                payload = self._client.get_json(url, timeout=self._timeout)
            except Exception:
            ...
            for article in articles:
                ...
                teams = tuple(
                    ...
                    if f"{league}:{team.lower()}" in followed
                )
                if not teams:
                    continue
