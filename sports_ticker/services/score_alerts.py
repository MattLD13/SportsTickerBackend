...
def normalize_last_play(raw, home_abbr='', away_abbr='', home_id=None, away_id=None, plays=None):
    """Flatten ESPN's ``situation.lastPlay`` into the fields the describer uses."""
    if not isinstance(raw, dict):
        return {}

    # MLB summary payloads commonly reduce situation.lastPlay to an id. The
    # useful text, scoreValue, and team are on the matching item in plays.
    # During a scoring update ESPN can already point lastPlay at the next
    # batter, so use the newest scoring play when the referenced item has no
    # scoring value.
    if isinstance(plays, list) and plays:
        play_id = str(raw.get('id') or '')
        resolved = next((p for p in plays if isinstance(p, dict) and str(p.get('id') or '') == play_id), None)
        if not isinstance(resolved, dict) or not resolved.get('text') or not _int_or_none(resolved.get('scoreValue')):
            scored = [p for p in plays if isinstance(p, dict) and _int_or_none(p.get('scoreValue'))]
            if scored:
                resolved = scored[-1]
    ...
    if not athlete:
        match = re.match(
            r'^(.+?)\s+(?:homered|singled|doubled|tripled|walked|hit|grounded|flied|reached|bunted|sacrificed)\b',
            str(raw.get('text') or ''),
            re.I,
        )
        if match:
