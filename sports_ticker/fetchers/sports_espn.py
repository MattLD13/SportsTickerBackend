    ...
    def fetch_single_league(self, league_key, config, conf, window_start_utc, window_end_utc, utc_offset, visible_start_utc, visible_end_utc):
        ...
        try:
            ...
            for e in events:
                ...
                if is_suspended:
                    s_disp = s_disp.split(',')[0].strip().title()
                ...
                # MLB summary payloads keep the live situation at the summary
                # root. Keep the competition fallback for older ESPN shapes.
                if league_key == 'mlb':
                    sit = data.get('situation') or comp.get('situation') or {}
                else:
                ...
                if league_key == 'mlb':
                    ...
                    _mlb_stats = self._mlb_extract_situation_stats(sit, _bat_obj, _pit_obj, data)
