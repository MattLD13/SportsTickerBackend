...
# TIMING
FAST_INTERVAL = 60    # Live stats every 60s
SLOW_INTERVAL = 600   # Vegas odds every 10m
SIM_COUNT = 50000 
...
def atomic_write(filename, data):
    """ Writes data to a temp file then renames it to prevent read errors """
...
class LiveESPNFetcher:
    ...
    def _parse_events(self, events):
        ...
        for e in events:
            ...
            elif state == 'post': time_rem_pct = 0.0
            if state == 'in': self._fetch_game_summary(e['id'], temp_map, time_rem_pct)
            ...
            for team in box.get('players', []):
                for stat_group in team.get('statistics', []):
                    ...
                    for athlete in stat_group.get('athletes', []):
                        name = athlete['athlete']['displayName']; stats = athlete['stats']; pts = 0.0
                        try:
                        ...
                        if name not in player_map: player_map[name] = {'score': 0.0, 'rem': time_rem_pct}
...
class OddsAPIFetcher:
    ...
    def load_cache(self):
        if os.path.exists(CACHE_FILE):
            try:
                with open(CACHE_FILE, 'r') as f: d = json.load(f); self.cache = d.get('payload', {})
    ...
    def fetch_fresh_props(self):
        ...
        try:
            r = requests.get(f"{self.base}/events?apiKey={self.key}")
            if r.status_code == 200:
                events = r.json(); self.cache = {'events': events}
                for e in events:
                    mkts = ",".join(PROP_MARKETS)
                    r2 = requests.get(f"{self.base}/events/{e['id']}/odds?apiKey={self.key}&regions=us&markets={mkts}&oddsFormat=american")
...
class FantasySimulator:
    ...
    def fuzzy_match_player(self, fantasy_name, api_player_names):
        ...
        best_match = None; highest_ratio = 0.0
        for api_name in api_player_names:
            ...
            if ratio > highest_ratio: highest_ratio = ratio; best_match = api_name
        return best_match if highest_ratio > 0.8 else None

    def get_projected_stats(self, player_name, event_props):
        stats = {}; agg_lines = {m: [] for m in PROP_MARKETS}; atd_odds = []
        ...
        for book in event_props.get('bookmakers', []):
            for market in book.get('markets', []):
                ...
                if key in agg_lines:
                    for o in market['outcomes']:
                        if o['description'] == player_name and 'point' in o: agg_lines[key].append(o['point'])
        count = 0
        for m, lines in agg_lines.items():
            if lines: stats[m] = statistics.mean(lines); count += 1
        if atd_odds:
            avg = statistics.mean(atd_odds); prob = 100/(avg+100) if avg>0 else abs(avg)/(abs(avg)+100)
    ...
    def calculate_fantasy_points(self, stats, rules):
        ...
        tot_skill_td = r_tds + rec_tds
        if tot_skill_td == 0 and 'anytime_td_prob' in stats: score += stats['anytime_td_prob'] * 6.0
        elif tot_skill_td > 0: score += (r_tds*6) + (rec_tds*6)
        else: score += ((r_yds/90.0)*6) + ((rec_yds/90.0)*6)
    ...
    def simulate_game(self, json_data, events_map, debug_data_list):
        ...
        rules = json_data['scoring_rules']; platform = json_data.get('league_settings', {}).get('platform', 'Fantasy')
        tag = "CBS" if "CBS" in platform else ("ESPN" if "ESPN" in platform else "Fantasy")
        payouts = LEAGUE_PAYOUTS.get(tag, {"win": 0, "loss": 0}); vol = LEAGUE_VOLATILITY.get(tag, 0.50)
        
        matchup = {'home': [], 'away': []}; debug = {"platform": tag, "home_team": home['name'], "away_team": away['name'], "players": []}

        for side in ['home', 'away']:
            team = home if side == 'home' else away
            for p in team['roster']:
                base_proj = p['proj']; base_std = base_proj * vol; source = "League"
                live = self.live_fetcher.get_player_live(p['name'])
                l_score = live['score'] if live else 0.0; rem_pct = live['rem'] if live else 1.0
                    ...
                    for eid, props in events_map.items():
                        ...
                        if match:
                            ...
                            if cnt > 0:
                                ...
                                base_std = vegas_proj * vol; source = "Vegas"
                            break
                
                if vegas_proj is not None and vegas_proj < (base_proj * 0.70):
                    vegas_proj = base_proj; source = "League (Vegas Low)"
                
                final_proj = (vegas_proj if vegas_proj else base_proj)
                final_mean = l_score + (final_proj * rem_pct)
                if p['pos'] in ['DST', 'K']: final_mean = l_score + (base_proj * rem_pct); source = "League"

                matchup[side].append({'name': p['name'], 'mean': final_mean, 'std': base_std})
                debug['players'].append({"name": p['name'], "pos": p['pos'], "team": side.upper(), "league_proj": round(l_score + (base_proj*rem_pct), 2), "my_proj": round(final_mean, 2), "source": source})
        ...
        return {
            "id": str(hash(home['name']+away['name']) % 100000), "status": tag, "home_abbr": "ME", 
            "home_score": f"{win_pct:.2f}", "away_abbr": "OTH", "away_score": f"{(100-win_pct):.2f}",
