    ...
    def fetch_live_stats(self):
        try:
            ...
            if r.status_code == 200:
                data = r.json(); self._parse_events(data.get('events', []))
        except: pass
    def _parse_events(self, events):
        temp_map = {}
        for e in events:
            status = e.get('status', {}); state = status.get('type', {}).get('state', 'pre')
            period = status.get('period', 1); time_rem_pct = 1.0
            if state == 'in':
                q_rem = 4 - period; time_rem_pct = (q_rem * 15) / 60.0
...
class OddsAPIFetcher:
    ...
    def save_cache(self):
        try:
            with open(CACHE_FILE, 'w') as f: json.dump({'timestamp': time.time(), 'payload': self.cache}, f)
        except: pass
    ...
    def simulate_game(self, json_data, events_map, debug_data_list):
        ...
        for side in ['home', 'away']:
            ...
            for p in team['roster']:
                ...
                l_score = live['score'] if live else 0.0
                rem_pct = live['rem'] if live else 1.0
                ...
                # === SAFETY FLOOR LOGIC ===
                # If Vegas is abnormally low (< 70% of league proj) likely due to missing props, revert to League.
                ...
                debug['players'].append({"name": p['name'], "pos": p['pos'], "team": side.upper(), 
