...
class SportsFetcher:
    def __init__(self, initial_weather_loc):
        ...
        # Explicitly separated team_params for FBS (Group 80) and FCS (Group 81)
        self.leagues = {
            ...
            if 'sports' in data:
                for sport in data['sports']:
                    for league in sport.get('leagues', []):
                        # Only include teams matching the FBS/FCS group
                        group_id = str(config['team_params'].get('groups', ''))
                        if group_id and str(league.get('groupId')) != group_id:
                            continue
...
# Keep everything else (get_real_games, Flask endpoints, etc.) unchanged
