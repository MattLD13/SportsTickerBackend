...
# ================= NEW: LOGO OVERRIDES =================
# These are the hardcoded logos you asked for.
LOGO_OVERRIDES = {
    # NHL
...
class SportsFetcher:
    def __init__(self):
        ...
        # ORIGINAL: Standard ESPN endpoints that work
        self.leagues = {
    ...
    # ================= ORIGINAL: Standard ESPN Fetch Logic =================
        ...
        for league_key, config in self.leagues.items():
            ...
            try:
                ...
                for event in data.get('events', []):
                    ...
                    # Logic: Keep if Active OR Matches Today's Date
                    ...
                    if keep_date:
                        ...
                        # --- LOGO URL FIX (Hardcoded check here or in serve_logo) ---
                        # We use standard URLs here, relying on serve_logo to fix them later
                        ...
                        game_obj = {
                            ...
                            'home_logo': home['team'].get('logo', ''), 'home_id': home.get('id'),
                            'away_abbr': away['team']['abbreviation'], 'away_score': away.get('score', '0'), 
                            'away_logo': away['team'].get('logo', ''), 'away_id': away.get('id'),
                            'situation': {}
                        }
                        ...
                        if status_state == 'in' and not is_halftime:
                            if 'football' in config['path']:
                                ...
                                # Basic PP logic from scoreboard
                                # Note: Full PP data usually needs 'gamecenter' endpoint, but sticking to basics for stability
                                pass 
...
# ================= UPDATED: IMAGE PROCESSING ENDPOINT =================
# This includes the "Auto-Crop" and "Padding" logic + Overrides
...
def serve_logo(league, abbr):
    ...
    # 2. If not overridden, find URL in current game data
    if not url:
    ...
    # 3. Fallback: Search all teams data
    if not url:
        target_leagues = []
        if 'ncf' in league: target_leagues = ['ncf_fbs', 'ncf_fcs']
