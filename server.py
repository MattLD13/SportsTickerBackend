    ...
    def get_real_games(self):
        ...
        if state['active_sports'].get('fantasy'):
            if os.path.exists(FANTASY_FILE):
                ...
                except (json.JSONDecodeError, ValueError): pass # Ignore corrupted reads
                except: pass
...
def background_updater():
    # Attempt to kill old instances to prevent port conflicts
...
def fantasy_dashboard():
    ...
    if os.path.exists(DEBUG_FILE):
        try:
            ...
            # If the file is half-written, show reloading instead of 502
            return "<meta http-equiv='refresh' content='1'><h3>Syncing Data...</h3>"
        except Exception as e:
            return f"<h3>Error: {str(e)}</h3>"
    else:
        return "<meta http-equiv='refresh' content='2'><h3>Initializing...</h3>"
    
    if not data or not isinstance(data, list): 
        return "<meta http-equiv='refresh' content='2'><h3>Waiting for Data...</h3>"
