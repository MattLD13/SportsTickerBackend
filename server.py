...
# ================= LOGO OVERRIDES =================
# Specific fixes for teams where the generic URL pattern fails
LOGO_OVERRIDES = {
    # NHL
    ...
    # NBA
    "UTA": "https://a.espncdn.com/i/teamlogos/nba/500/utah.png", # Jazz
...
class SportsFetcher:
    ...
    def get_games(self):
        ...
        if state['debug_mode'] and state['custom_date']: target_date_str = state['custom_date']
        ...
        for league, cfg in self.leagues.items():
            ...
            try:
                ...
                for event in data.get('events', []):
                    ...
                    # LOGO LOGIC (Fallbacks applied in serve_logo, here we just pass raw data)
                    ...
                    # Formatting Status
                    if g['status']:
                        g['status'] = g['status'].replace("Final", "FINAL").replace(" EST", "").replace(" EDT", "").replace("/OT", "")
...
def serve_logo(league, abbr):
    ...
    # 2. If no override, CONSTRUCT UNIVERSAL ESPN URL
    # This fixes the "Missing Logos" for everyone else without needing a massive list
    if not url:
        # Map our internal league names to ESPN CDN folders
        cdn_league = league
        if "ncf" in league: cdn_league = "ncaa"
        
        # ESPN CDN Format: https://a.espncdn.com/i/teamlogos/[LEAGUE]/500/[ABBR].png
