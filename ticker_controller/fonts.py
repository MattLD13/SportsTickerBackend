...
# Small centred "v" used as a versus separator in matchups (e.g. baseball
# "AWAY v HOME"). Kept distinct from the full-height capital 'V' so team names
# and headers render a normal letter while a lowercase 'v' stays small.
...
def draw_hybrid_text(draw, x, y, text_str, color):
    # Keep case so a lowercase 'v' (versus separator) stays the small glyph
    # while everything else is upper-cased for the caps-only font lookup.
    text_str = normalize_special_chars(str(text_str))
    x_cursor = x
    for char in text_str:
        if char == '~':
            x_cursor += 2
