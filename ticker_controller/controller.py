...
class TickerStreamer(SportsMixin, WeatherMixin, GolfMixin, MusicMixin, FlightMixin, MiscMixin, IndycarMixin, F1Mixin, NascarMixin):
    ...
    def get_item_width(self, game):
        ...
        if t == 'racing' or str(s).lower() in ('indycar', 'f1', 'nascar'):
            return PANEL_W if self.mode in ('indycar', 'f1', 'nascar') or self.is_full_display() else 128 + GAME_SEPARATOR_W
            ...
            try:
                ...
                if current_hash != last_hash:
                    ...
                    for g in other_games:
                        ...
                        is_indycar_fullscreen = is_indycar and (self.mode in ('indycar', 'f1', 'nascar') or self.is_full_display())
